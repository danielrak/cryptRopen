...
* the input file is written to `output_path` re-encoded to the output
  format implied by `encrypted_file` (e.g. csv → csv, parquet → parquet);
...
`crypt_data()` does **not** support this: calling it without anything
to encrypt is treated as misuse and raises an explicit error pointing
to `dplyr` / `rio` for plain column dropping or format conversion.
