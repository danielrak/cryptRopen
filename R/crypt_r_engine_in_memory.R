...
#' report. Kept separate from the dispatcher so that the historical
#' code path stays strictly unchanged while the streaming engines
#' evolve.
#'
#' @section FROZEN file outputs:
...
#'
#' @section No globalenv() pollution:
...
#' `eval(parse(text = ...))`. Both patterns have been removed: the data
#' flows through local variables only; the correspondence table is
#' stored in the package-private `.cryptRopen_env` (retrievable via
#' [get_correspondence_tables()]) and on disk, mirroring the streaming
#' engines. File outputs are byte-identical to the historical
#' implementation on matching inputs — baseline unchanged.
#'
...
#'   `tc_df`, `metrics` (see `.make_row_result()`). The return value is
#'   additive — the mirai dispatcher ships it back to
#'   `cryptR_collect()`. Tests that call the engine for its side
#'   effects only can safely ignore it.
#' @noRd
.process_mask_row_in_memory <- function(mask_row, input_path, output_path, intermediate_path,
  ...
  # export. Error messages are accumulated in `errs` so the recap log
  # in `cryptR_collect()` can report what went wrong per row.
  start_time <- Sys.time()
  errs <- character(0)
